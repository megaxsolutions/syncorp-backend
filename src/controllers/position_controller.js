...
export const create_position = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [insert_data_position] = await db.promise().query(sql, [position_name]);
...
export const update_position = asyncHandler(async (req, res) => {
    const { position_name } = req.body;
    const { position_id } = req.params; // Assuming department_id is passed as a URL parameter


    try {
        const sql = 'UPDATE positions SET position = ? WHERE id = ?';
        const [result] = await db.promise().query(sql, [position_name, position_id]);
...
export const delete_position = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.promise().query(sql, [position_id]);
