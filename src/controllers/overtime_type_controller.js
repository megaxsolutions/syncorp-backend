...
export const delete_overtime_type = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const sql2 = 'SELECT * FROM admin_login WHERE JSON_CONTAINS(user_level, ?)';
        const sql3 = 'INSERT INTO logs (details, datetime, user_level, emp_ID, is_read) VALUES (?, ?, ?, ?, ?)';
        const sql4 = 'SELECT * FROM overtime_request WHERE ot_type = ?'; // Use a parameterized query
        const sql5 = 'DELETE FROM overtime_type WHERE id = ?';
        ...
        const [data_overtime_request] = await db.query(sql4, [overtime_type_id]);
        ...
        const insert_logs_admin_level = await Promise.all(
            select_data_admin_login.map(async (admin_login) => {
                await db.query(sql3, ['Overtime type has been deleted: ' + data_overtime_type[0]['type'], storeCurrentDateTime(0, 'hours'), 1, admin_login.emp_ID, 0]);
                return admin_login.emp_ID;
            })
        );
        ...
        if(data_overtime_request.length == 0) {
            const [result] = await db.query(sql5, [overtime_type_id]);
