...
export const create_cutoff = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [insert_data_site] = await db.promise().query(sql, [start_date, end_date ]);
...
export const update_cutoff = asyncHandler(async (req, res) => {
    ...
    const { cutoff_id } = req.params; // Assuming department_id is passed as a URL parameter


    try {
        const sql = 'UPDATE cutoff SET startDate = ?, endDate = ?, status = ? WHERE id = ?';
        const [result] = await db.promise().query(sql, [start_date, end_date, status, cutoff_id]);
...
export const delete_cutoff  = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.promise().query(sql, [cutoff_id]);
