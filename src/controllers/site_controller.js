...
export const create_site = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [insert_data_site] = await db.promise().query(sql, [site_name]);
...
export const update_site = asyncHandler(async (req, res) => {
    const { site_name } = req.body;
    const { site_id } = req.params; // Assuming department_id is passed as a URL parameter


    try {
        const sql = 'UPDATE sites SET siteName = ? WHERE id = ?';
        const [result] = await db.promise().query(sql, [site_name, site_id]);
...
export const delete_site = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.promise().query(sql, [site_id]);
